...
const wolframApiKey = process.env.wolfram;
const kursnaListaApiKey = process.env.lista;
...
controller.hears(['schedule'], 'direct_message', function(bot, message) {
    ...
    controller.storage.users.get(message.user, function(err, user) {
        if (user && user["answer1"] && user["answer2"] && user["answer3"] && user["answer4"]) {
            ...
            if(user["answer1"] != 'nothing') message_to_send += '9-12h: ' + user["answer1"] + '\n';
            if(user["answer2"] != 'nothing') message_to_send += '12-15h: ' + user["answer2"] + '\n';
            if(user["answer3"] != 'nothing') message_to_send += '15-18h: ' + user["answer3"] + '\n';
            if(user["answer4"] != 'nothing') message_to_send += '18-21h: ' + user["answer4"] + '\n';
